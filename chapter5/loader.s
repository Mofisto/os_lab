...
;构建gdt及其内部的描述符
   ...
   ; 当前偏移loader.bin文件头0x200字节,loader.bin的加载地址是0x7e00,
   ; 故total_mem_bytes内存中的地址是0x8000.将来在内核中咱们会引用此地址
...
WHILE:
   jmp WHILE  ;如果真的 ret 回来，就在这里无限循环
...
.clear_page_dir:
   ...
   inc esi
   loop .clear_page_dir ;这里的数据有点不对了。
